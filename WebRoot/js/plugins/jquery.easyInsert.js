...
;(function($){
	$.fn.extend({
		"easyinsert": function(o){
			...
			//把属性拼成数组（这步不知道是否可以优化？）
			var arrCfg = new Array(o.name, o.type, o.value, o.maxlength, o.className,o.onclick);
						...
						$.each(arr[0], function(i) {
							switch ( arr[1][i] ) {
								...
								case "custom"://自定义内容，支持html
									...
									break;
								case "wk":
									$Item.append( "<input type='text' id='it"+len+"' name='intervlTimes' onclick=\"WdatePicker({dateFmt:'HH:mm',isShowClear:false,readOnly:true})\">--<input type='text' name='intervlTimes' onclick=\"WdatePicker({dateFmt:'HH:mm',minDate:'#F{$dp.$D(\\'it"+len+"\\')}',isShowClear:false,readOnly:true})\">" );
									break;
								default://默认是input
									$("<input>", {//jQuery1.4新增方法
										name: arr[0][i],
